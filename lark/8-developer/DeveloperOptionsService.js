...
export class DeveloperOptionsService {
    ...
    toggleFPS(show) {
        if (show) {
            if (!this.fpsCounter) {
                ...
                this.fpsCounter.className = 'omni-fps-counter';
    ...
    /**
     * Consumes PresentationEffectsService.getDiagnosticsSnapshot() for developer telemetry reporting.
     * @param {Object} registry - ServiceRegistry instance
     * @returns {Object|null} Telemetry snapshot or null
     */
    getPresentationDiagnostics(registry) {
        if (!registry) return null;
        const presentationEffects = registry.get('PresentationEffectsService');
        return (presentationEffects && typeof presentationEffects.getDiagnosticsSnapshot === 'function')
            ? presentationEffects.getDiagnosticsSnapshot()
            : null;
    }
