...
export function omni_preferenceItem2(text_primary, text_secondary, text_tertiary, actions) {
    return `
        <div class="layout-v flex-align-start flex-gap-16">
            <div class="layout-v flex-1 flex-gap-2">
                <p>${text_primary}</p>
                ${text_secondary ? `<small>${text_secondary}</small>` : ''}
                ${text_tertiary ? `<small class="er">${text_tertiary}</small>` : ''}
...
export function omni_imagelistItem(src, text_primary = '', text_secondary = '', text_tertiary = '', omni_components = '') {
    const imgSrc = src ? src : 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg"/>'; // Fallback SVG if src is empty
    
    return `
        <div class="layout-h flex-align-center flex-gap-16">
            <img src="${imgSrc}" alt="${text_primary}" />
            <div class="layout-v flex-1 flex-gap-2">
                ${text_primary ? `<p>${text_primary}</p>` : ''}
                ${text_secondary ? `<small>${text_secondary}</small>` : ''}
                ${text_tertiary ? `<small class="er">${text_tertiary}</small>` : ''}
                ${omni_components ? `${omni_components}` : ''}
            </div>
...
export function omni_statusBadge(text, variant = 'info') {
    return omni_badge(text, variant);
}
...
export function omni_toolbar(omni_components, extraClass = '') {
    return `
        <div class="comp-toolbar layout-h flex-align-center flex-gap-8${extraClass}">
            ${omni_components}
        </div>
    `;
}