...
export class ApplicationDatabaseService {
    ...
    async save() {
        ...
        const systemContext = securityService.getSystemContext();
        if (!fileService.exists('/system/installation.json', { context: systemContext })) {
            return;
        }
    ...
    _normalizeManifest(app) {
        return {
            ...
            runtime: {
                ...
                startup: app.startup !== undefined ? app.startup : (app.runtime && app.runtime.startup !== undefined ? app.runtime.startup : false),
            ...
            install: {
                ...
                installDate: app.installDate ? app.installDate : (new Date()).toISOString(),
    ...
    onChange(callback) {
        ...
        EventBus.on('application.database.changed', handler);
        return () => EventBus.off('application.database.changed', handler);
    }
}
