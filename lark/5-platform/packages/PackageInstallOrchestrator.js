 ...
 */
export class PackageInstallOrchestrator {
    ...
    async installPackage(packagePath, pid = null) {
        ...
        EventBus.emit('package.install.started', { 
            severity: 'Info', 
            source: 'PackageInstallOrchestrator', 
    ...
    async _performInstall(packageData, trustMetadata = null) {
        ...
        try {
            ...
            throw new Error('PackageInstallOrchestrator: Invalid package format. Expected JSON.');
        }

        if (!pkg.manifest) {
            throw new Error('PackageInstallOrchestrator: Package is missing a valid manifest.');
        }

        const diagnostics = this.validator.validate(pkg.manifest);
        if (this.validator.hasErrors(diagnostics)) {
            const errors = diagnostics.filter(d => d.severity === 'error');
            throw new Error(`PackageInstallOrchestrator: Manifest validation failed with errors: ${errors.map(e => e.message).join(', ')}`);
        }

        // Log warnings
        const warnings = diagnostics.filter(d => d.severity === 'warning');
        if (warnings.length > 0) {
            console.warn(`PackageInstallOrchestrator: Manifest validation warnings for ${pkg.manifest.id || 'unknown'}:`, warnings);
        }
    ...
    async _performUninstall(appId) {
        const appDbService = this.serviceRegistry.get('ApplicationDatabaseService');
        if (!appDbService) throw new Error('PackageInstallOrchestrator: ApplicationDatabaseService not found.');
        
        const app = appDbService.getAppById(appId);
        if (!app) {
            throw new Error(`PackageInstallOrchestrator: App ${appId} is not installed.`);
        }
        ...
        if (trustService) {
            ...
            if (trustMeta && trustMeta.state === 'BUILT_IN') {
                throw new Error(`PackageInstallOrchestrator: Cannot uninstall built-in system application ${appId}.`);
            }
        }
