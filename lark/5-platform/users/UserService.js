 ...
 * Standardizes domain operations (createUser, updateUser, deleteUser) using Result Object contracts.
 * Pure read-only getters (getUser, getUsers) return native values.
 */
export class UserService {
    ...
    createUser(username, displayName, password, options = {}) {
        ...
        if (!isFirstUser && securityPolicy) {
            if (securityPolicy.canManageUsers(context) !== 'ALLOW') {
                return Result.failure('PERMISSION_DENIED', 'Permission denied: Cannot create user accounts.');
            }
        }
        
        // Step 1: Validate Uniqueness
        if (this.userManager.getUsers().find(u => u.username === username)) {
            return Result.failure('USER_EXISTS', `User account '${username}' already exists.`);
        }
        ...
        try {
            ...
            return Result.success(user, `User account '${username}' created successfully.`);
        } catch (e) {
            // Rollback
            ...
            return Result.failure('CREATE_USER_FAILED', `Failed to create user '${username}': ${e.message}`);
        }
    }
    ...
    async renameUser(oldUsername, newUsername, options = {}) {
        ...
        try {
            ...
            if (fileService && fileService.exists(oldHome, { context: systemContext })) {
                ...
                if (typeof fileService.repairMetadata === 'function') {
                    try {
                        fileService.repairMetadata(newHome, { ownerOverride: newUsername, context: systemContext, recursive: true });
                    } catch (e) {
                        console.error(`[UserService] Failed to repair metadata for new home directory ${newHome}:`, e);
                    }
                }
    ...
    onCreated(callback) { return this._bindEvent('user.created', callback); }
    onDeleted(callback) { return this._bindEvent('user.deleted', callback); }
    onRenamed(callback) { return this._bindEvent('user.renamed', callback); }
    onProfileChanged(callback) { return this._bindEvent('user.profile.changed', callback); }
    onPasswordChanged(callback) { return this._bindEvent('user.password.changed', callback); }
}
