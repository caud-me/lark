...
export class UserSettingsManager {
    constructor() {
        ...
        this.defaults = {
            ...
            'appearance.visualEffectsEnabled': true
        };
    }
     ...
     */
    get(username, key) {
        let settings = username ? this.userSettings.get(username) : null;
        if (!settings && this.userSettings.size > 0) {
            settings = this.userSettings.values().next().value;
        }
        if (!settings) {
