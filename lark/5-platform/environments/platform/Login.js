            ...
            if (users.length === 0) {
                card.innerHTML = `
                    <div style="text-align: center; margin-bottom: 32px;">
                        <div style="font-size: 3.5rem; margin-bottom: 16px; font-family: 'sfi';"><i>&#xE77B;</i></div>
                ...
                sortedUsers.forEach(u => {
                    ...
                    const avatar = (profile && profile.avatar) ? profile.avatar : '<i>&#xE77B;</i>';
                    ...
                    btn.onclick = async () => {
                        try {
                            ...
                            const loginResult = SessionService.login(u.username);
                            if (loginResult && !loginResult.success) {
                                await DialogService.alert(loginResult.message || 'Login failed.', 'Login Error');
                                return;
                            }
                            cleanup();
                        } catch (e) {
