...
import { BrowserStorageService } from '../browser/BrowserStorageService.js';
...
export class FileService {
    ...
    _checkAccess(path, operation, options = {}) {
        const context = this._resolveContext(options);
        const normPath = this.lrfs._normalizePath(path);
        ...
        if (securityPolicy) {
            ...
            if (this.lrfs.exists(normPath)) {
                metadata = this.lrfs.getMetadata(normPath);
            }
            
            const parentPath = this._getParentPath(normPath);
            if (parentPath && this.lrfs.exists(parentPath)) {
                parentMetadata = this.lrfs.getMetadata(parentPath);
            }

            const result = securityPolicy.canAccessPath(context, normPath, operation, metadata, parentMetadata);
            
            if (result === 'DENY') {
                EventBus.emit('permission.denied', { severity: 'Warning', source: 'FileService', message: `${operation} denied to ${normPath} by SecurityPolicy` });
                throw new Error(`Permission denied: Cannot ${operation} ${normPath}`);
            } else if (result === 'REQUIRES_ELEVATION') {
                EventBus.emit('permission.denied', { severity: 'Warning', source: 'FileService', message: `${operation} to ${normPath} requires elevation` });
                throw new Error(`Permission denied: Administrator privileges required to ${operation} ${normPath}`);
            }
        }
    }
    ...
    canAccess(path, operation, options = {}) {
        ...
        const result = securityPolicy.canAccessPath(context, normPath, operation, metadata, parentMetadata);
        // console.log('[Diagnostic] FileService.canAccess - Path:', normPath, 'Op:', operation, 'Context:', context, 'Result:', result);
        return result === 'ALLOW';
    }
    ...
    async open(path, options = {}) {
        ...
        if (this.isDirectory(path)) {
            ...
            if (intentService) {
                return await intentService.launchWithIntent('sys.finder', { action: 'open-file', path }, { args: [path] });
            }
            return await processService.startProcess('sys.finder', { args: [path] });
        }
    ...
    /**
     * Gets active low-level storage driver type ('LocalStorage' or 'IndexedDB').
     * @returns {string}
     */
    getDriverType() {
        if (this.lrfs && this.lrfs.driver) {
            if (this.lrfs.driver.type) {
                return this.lrfs.driver.type;
            }
            if (this.lrfs.driver.dbName || this.lrfs.driver.constructor.name === 'IndexedDBStorageDriver') {
                return 'IndexedDB';
            }
        }
        return 'LocalStorage';
    ...
    /**
     * Exposes unified storage capability contract object to callers.
     * Consumes low-level storage capabilities from LRFS driver without assumptions.
     */
    async getStorageInfo() {
        try {
            const driverCaps = (this.lrfs && typeof this.lrfs.getCapabilities === 'function') 
                ? await this.lrfs.getCapabilities() 
                : {
                    type: 'Storage',
                    label: 'Storage',
                    name: 'Disk',
                    usageBytes: this.getUsage()
                };

            const browserCaps = await BrowserStorageService.getStorageCapabilities();

            const usageBytes = typeof driverCaps.usageBytes === 'number' ? driverCaps.usageBytes : (this.getUsage() || 0);
            const originUsageBytes = typeof browserCaps.usageBytes === 'number' ? browserCaps.usageBytes : null;
            const isLocalStorage = (driverCaps.type === 'LocalStorage' || this.getDriverType() === 'LocalStorage');
            
            let capacityBytes = (typeof driverCaps.capacityBytes === 'number' && driverCaps.capacityBytes > 0)
                ? driverCaps.capacityBytes
                : (typeof browserCaps.quotaBytes === 'number' && browserCaps.quotaBytes > 0 ? browserCaps.quotaBytes : (this.getCapacity() || 5242880));

            if (!capacityBytes || capacityBytes <= 0) {
                capacityBytes = 5242880; // 5 MB fallback
            }

            const rawPercent = (usageBytes / capacityBytes) * 100;
            let percentUsed = Math.min(100, Math.round(rawPercent * 10) / 10);
            if (usageBytes > 0 && percentUsed === 0) {
                percentUsed = 0.1;
            }
            ...
            return Result.success({
                driverType: driverCaps.type || 'Storage',
                driverLabel: driverCaps.label || 'Virtual Storage Volume',
                diskName: driverCaps.name || 'Lark System Disk',
                usageBytes,
                originUsageBytes,
                capacityBytes,
                formattedUsage: formatBytes(usageBytes),
                formattedOriginUsage: formatBytes(originUsageBytes),
                formattedCapacity: formatBytes(capacityBytes),
                percentUsed,
                persisted: browserCaps.persisted
            });
        } catch (e) {
            return Result.failure('STORAGE_INFO_FAILED', e.message);
        }
    }
    ...
    repairMetadata(path, options = {}) {
        ...
        if (options.ownerOverride && this.lrfs.exists(path)) {
            ...
            if (options.recursive && this.isDirectory(path)) {
                try {
                    const children = this.listDirectory(path, options) || [];
                    for (const child of children) {
                        this.repairMetadata(child.path, options);
                    }
                } catch (e) {
                    console.error(`[FileService] Failed to recursively repair metadata for ${path}:`, e);
                }
            }
        }
    }
    ...
    /**
     * Returns high-level storage overview snapshot derived directly via KernelStorageAPI.
     */
    getStorageInfo() {
        let storageApi = null;
        if (this.registry) {
            storageApi = this.registry.get('KernelStorageAPI');
        }

        const usageBytes = storageApi ? storageApi.getUsage() : 0;
        const capacityBytes = storageApi ? storageApi.getCapacity() : 5242880;
