...
export function registerDiskCommands(cmdService) {
    cmdService.registerCommand({
        ...
        handler: async (args, context) => {
            ...
            if (fs && typeof fs.getStorageInfo === 'function') {
                const storageContract = await fs.getStorageInfo();
                if (storageContract && storageContract.success) {
                    const d = storageContract.data;
                    let out = `Storage Engine: ${d.driverLabel} (${d.diskName})\n`;
                    out += `Usage: ${d.formattedUsage} / ${d.formattedCapacity}`;
                    if (d.percentUsed !== null) out += ` (${d.percentUsed}% used)`;
                    if (d.formattedOriginUsage) out += `\nBrowser Origin Usage: ${d.formattedOriginUsage}`;
                    return out;
                }
            }
