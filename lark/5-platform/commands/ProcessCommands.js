...
export function registerProcessCommands(cmdService) {
    cmdService.registerCommand({
        ...
        handler: async (args, context) => {
            const appService = cmdService.registry.get('ApplicationService');
            if (!appService) throw new Error('ApplicationService not found');
            const apps = appService.getInstalledApplications()
                .filter(a => a.type !== 'platform-environment' && a.type !== 'system-service');
    ...
    cmdService.registerCommand({
        ...
        handler: async (args, context) => {
            ...
            const ps = cmdService.registry.get('ProcessService');
            const pid = await ps.startProcess(args[0], { sessionId: context.sessionId });
