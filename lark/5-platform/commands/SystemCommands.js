...
export function registerSystemCommands(cmdService) {
    cmdService.registerCommand({
        ...
        handler: async (args, context) => {
            const sysLog = cmdService.registry.get('LogService');
    ...
    cmdService.registerCommand({
        name: 'about',
        description: 'Show OS or Application information',
        usage: 'about [appId]',
        handler: async (args, context) => {
            if (args[0]) {
                const appSvc = cmdService.registry.get('ApplicationService');
                const app = appSvc.getApplication(args[0]);
                if (!app) return `Application '${args[0]}' not found.`;
                return `Application: ${app.title}\nVersion: ${app.version || 'Unknown'}\nAuthor: ${app.author || 'Unknown'}\nCategory: ${app.category || 'Unknown'}\nDescription: ${app.description || ''}\nSingleton: ${app.singleton ? 'Yes' : 'No'}`;
            }
    ...
    cmdService.registerCommand({
        ...
        handler: async (args, context) => {
            ...
            const appsCount = cmdService.registry.get('ApplicationService').getInstalledApplications().length;
    ...
    cmdService.registerCommand({
        name: 'window-center',
        description: 'Center the focused or top window in the active workspace',
        usage: 'window-center',
        handler: async (args, context) => {
            const windowService = cmdService.registry.get('WindowService');
            if (!windowService) return 'WindowService unavailable.';
            const mruList = windowService.getMRUWindowList();
            const active = mruList.find(w => w.focused) || mruList[0];
            if (!active) return 'No active window to center.';
            windowService.centerWindow(active.id);
            return `Centered window '${active.title || active.id}'.`;
        }
    });
}
