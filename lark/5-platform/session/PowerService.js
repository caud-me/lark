...
export class PowerService {
    ...
    async reboot(options = { mode: BootMode.NORMAL }) {
        ...
        // Set active environment to ShutdownPlatformEnvironment so session.loggedOut listener ignores logout transitions
        try {
            const bootOrchestrator = this.registry ? this.registry.get('BootOrchestrator') : null;
            if (bootOrchestrator && typeof bootOrchestrator.transitionTo === 'function') {
                const { ShutdownPlatformEnvironment } = await import('../boot/BootOrchestrator.js');
                const shutdownEnv = new ShutdownPlatformEnvironment(this.registry);
                await bootOrchestrator.transitionTo(shutdownEnv);
            }
        } catch (e) {
            console.warn('[PowerService] Could not transition to ShutdownPlatformEnvironment:', e);
        }
