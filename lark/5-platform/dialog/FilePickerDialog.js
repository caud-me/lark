...
//                 icon.innerHTML = child.type === 'directory' ? '<i>&#xE8B7;</i>' : '<i>&#xE8A5;</i>';
...
export async function showFilePickerDialog(dialogService, fileService, options = {}) {
    ...
    newFolderBtn.className = 'shell-fd-button';
    newFolderBtn.innerHTML = '<i class="shell-fd-icon">&#xE109;</i>'; // Navigation Plus Glyph
    ...
    pathLabel.style.flex = '1';
    pathLabel.style.fontFamily = 'monospace';
    pathLabel.classList.add('omni-path-box');
    pathLabel.style.overflow = 'hidden';
    pathLabel.style.textOverflow = 'ellipsis';
    pathLabel.style.whiteSpace = 'nowrap';
    ...
    const list = document.createElement('div');
    list.className = 'layout-v omni-file-list';
    list.style.flex = '1';
    list.style.overflowY = 'auto';
    ...
    const renderList = () => {
        ...
        const updatePrimaryButtonState = () => {
            ...
            if (overlayBox) {
                ...
                if (primaryConfirmButton) {
                    ...
                    if (canProceed && requireWriteAccess) {
                        let targetPath = currentPath;
                        if (mode === 'directory' && selectedFile && selectedFile.type === 'directory') {
                            targetPath = currentPath === '/' ? `/${selectedFile.name}` : `${currentPath}/${selectedFile.name}`;
                        } else if (mode === 'save') {
                            const name = filenameInput.value.trim();
                            targetPath = name ? (currentPath === '/' ? `/${name}` : `${currentPath}/${name}`) : currentPath;
                        }
                        
                        if (typeof fileService.canAccess === 'function') {
                            // Check read access for the target path (navigation)
                            if (fileService.canAccess(targetPath, 'read', { pid: options.pid }) === false) {
                                canProceed = false;
                            } else {
                                // For selection, check write access
                                if (fileService.canAccess(targetPath, 'write', { pid: options.pid }) === false) {
                                    canProceed = false;
                                }
                            }
                        }
        ...
        filenameInput.oninput = updatePrimaryButtonState;

        try {
            // console.log('[Diagnostic] FilePickerDialog calling FileService.listDirectory for path:', currentPath, 'with options:', options);
            const children = fileService.listDirectory(currentPath, { pid: options.pid }) || [];
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                item.onclick = () => {
                    ...
                    for (let k = 0; k < allRows.length; k++) {
                        allRows[k].classList.remove('omni-row-selected');
                    }
                    item.classList.add('omni-row-selected');
                    selectedFile = child;
            ...
            if (list.children.length === 0) {
                ...
                emptyContainer.className = 'shell-fd-small-text padding-8 omni-text-muted';
            ...
            errorDiv.className = 'shell-fd-small-text omni-text-error';
            errorDiv.style.padding = '16px';
            errorDiv.style.textAlign = 'center';
            errorDiv.textContent = `Error reading directory: ${err.message}`;
    ...
    newFolderBtn.onclick = async () => {
        if (typeof fileService.canAccess === 'function' && fileService.canAccess(currentPath, 'write', { pid: options.pid }) === false) {
            dialogService.alert('You do not have permission to create a folder here.', 'Permission Denied');
            return;
        }
        
        const folderName = await dialogService.prompt('Enter new folder name:', 'New Folder', 'Create Folder');
        if (folderName) {
            ...
            if (cleanName) {
                ...
                if (!fileService.exists(targetPath)) {
                    try {
                        fileService.createDirectory(targetPath, { pid: options.pid });
                        renderList();
                    } catch (err) {
                        dialogService.alert(`Failed to create folder: ${err.message}`, 'Error');
                    }
                } else {
                    dialogService.alert('A file or folder with that name already exists.', 'Error');
                }
            }
        }
    };
    ...
    return new Promise(async (resolve) => {
        const dialogPromise = dialogService.show({
            ...
            buttons: [
                { label: 'Cancel', result: null },
                { label: mode === 'save' ? 'Save' : (mode === 'directory' ? 'Select Folder' : 'Open'), result: 'confirm', primary: true }
            ]
        });
