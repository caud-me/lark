...
import { ProcessPolicy } from '../../4-policies/ProcessPolicy.js';
 ...
 * Standardizes application launches via Result Object capabilities (launch).
 */
export class ProcessService {
    ...
    /**
     * OS Capability: Launches an application and returns a Standard Result Object containing processId.
     * @param {string} appId 
    ...
    async launch(appId, options = {}) {
        try {
            ...
            if (!appInfo) {
                return Result.failure('APP_NOT_FOUND', `Application '${appId}' not found.`);
            }

            const pid = await this.startProcess(appId, options);
            if (pid) {
                return Result.success({ processId: pid, windowId: null }, `Application '${appId}' launched successfully.`);
            }
            return Result.failure('LAUNCH_FAILED', `Failed to start process for application '${appId}'.`);
        } catch (e) {
            return Result.failure('LAUNCH_DENIED', e.message);
        }
    }
    ...
    async startProcess(appId, options = {}) {
        ...
        if (appInfo.singleton) {
            ...
            if (existingProc) {
                ...
                if (ownWindows && ownWindows.length > 0) {
                    ...
                    // Process has no open windows — terminate zombie process to allow clean re-launch
                    try {
                        this.terminateProcess(existingProc.pid);
                    } catch (e) {
        ...
        try {
            ...
            const scopedRegistry = new ScopedServiceRegistry(this.serviceRegistry, pid, options || {});
            if (module.default && typeof module.default.run === 'function') {
                // Do not await run so the app can live in the background
                module.default.run(scopedRegistry, pid, options).catch(e => {
    ...
    async startDesktopEnvironment(desktopEnv, options = {}) {
        ...
        try {
            ...
            const scopedRegistry = new ScopedServiceRegistry(this.serviceRegistry, pid, options);
            if (module.default && typeof module.default.run === 'function') {
                module.default.run(scopedRegistry).catch(e => {
    ...
    onStarted(callback) { return this._bindEvent('process.started', callback); }
    onTerminated(callback) { return this._bindEvent('process.terminated', callback); }
}
